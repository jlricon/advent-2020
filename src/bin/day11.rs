...
fn count_adjacent_seats(map: &HashMap<(usize, usize), Tile>, coord: (usize, usize)) -> u32 {
    (-1..=1)
        ...
        .map(|(a, b): (i32, i32)| {
            ...
            match maybe_coords {
                (a, b) if a < 0 || b < 0 => None,
                (a, b) => map.get(&(a as usize, b as usize)),
            }
        })
...
fn count_far_seats(map: &HashMap<(usize, usize), Tile>, coord: (usize, usize)) -> u32 {
    (-1..=1)
        .cartesian_product(-1..=1)
        .filter(|(a, b)| !(*a == 0 && *b == 0))
        .map(|(a, b): (i32, i32)| {
...
fn count_occupied_seats(map: &HashMap<(usize, usize), Tile>, nrow: usize, ncol: usize) -> u32 {
    (0..=nrow)
        .cartesian_product(0..=ncol)
        .map(|(row, col)| match map.get(&(row, col)).unwrap() {
            Tile::Occupied => 1,
            _ => 0,
        })
        .sum()
...
fn change_seat(
    ...
    y: usize,
    n_for_change: u32,
    seat_count_fn: fn(&HashMap<(usize, usize), Tile>, (usize, usize)) -> u32,
) -> Tile {
    let this_tile = map.get(&(x, y)).unwrap();
    match this_tile {
...
fn step_map(
    ...
    n_for_change: u32,
    seat_count_fn: fn(&HashMap<(usize, usize), Tile>, (usize, usize)) -> u32,
) -> (HashMap<(usize, usize), Tile>, bool) {
    let mut new_map = HashMap::new();
    let mut are_equal = true;
    (0..=nrow)
        .cartesian_product(0..=ncol)
        .for_each(|(row, col)| {
            let this_tile = map.get(&(row, col)).unwrap();
            let new_tile = change_seat(map, row, col, n_for_change, seat_count_fn);
            new_map.insert((row, col), new_tile);
...
fn main() {
    ...
    let nseats = loop {
        let map_and_eq = step_map(&part1_map, nrow, ncol, 4, count_adjacent_seats);
        if map_and_eq.1 {
            break count_occupied_seats(&map_and_eq.0, nrow, ncol);
        }
    ...
    let nseats = loop {
        let map_and_eq = step_map(&part2_map, nrow, ncol, 5, count_far_seats);
        if map_and_eq.1 {
            break count_occupied_seats(&map_and_eq.0, nrow, ncol);
        }
